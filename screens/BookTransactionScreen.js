...
import { Text, View, TextInput, TouchableOpacity, KeyboardAvoidingView, ToastAndroid, Alert } from 'react-native';
...
export default class BookTransactionScreen extends React.Component {
  ...
  initiateBookReturn = async() =>{
    ...
    db.collection("students").doc(this.state.scannedStudentId).udate({
  ...
  checkStudentEligibiltyForReturn = async () => {
    ...
    .where("bookId", "==", this.state.scanedBookId)
    .limit(1)
    .get();
    var isStudentEligible = "";
    transactionRef.docs.map(doc => {
      ...
      if (lastBOOKtransaction.studentId === this.state.scannedStudentId) {
        ...
        isStudentEligible = false;
        Alert.alert("this book wasnt issued by the student!");
        this.setState({
          scannedStudentId: "",
          scannedBookId: ""
        });
